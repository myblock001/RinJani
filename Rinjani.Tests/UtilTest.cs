...
namespace Rinjani.Tests
{
    ...
    public class UtilTest
    {
        ...
        public void MergeTest()
        {
            ...
            {
                return new Quote {Price = i, Volume = 1, Broker = Broker.Zb, Side = QuoteSide.Ask};
            }
        ...
        public void MergeTest2()
        {
            var quotes = new List<Quote>
            {
                new Quote(Broker.Hpx, QuoteSide.Ask, 14, (decimal) 0.1, (decimal) 0.1),
                new Quote(Broker.Hpx, QuoteSide.Ask, 11, (decimal) 0.2, (decimal) 0.2),
                new Quote(Broker.Hpx, QuoteSide.Bid, 12, (decimal) 0.1, (decimal) 0.1),
                new Quote(Broker.Zb, QuoteSide.Ask, 11, (decimal) 0.01, (decimal) 0.01),
                new Quote(Broker.Zb, QuoteSide.Bid, 13, (decimal) 0.01, (decimal) 0.01)
            };

            var step = 5;
            var r = quotes.Merge(step).OrderBy(q => q.Price).ToList();
            Assert.AreEqual(4, r.Count);
            var ql = r.Where(q => q.Broker == Broker.Hpx && q.Side == QuoteSide.Ask && q.Price == 15).ToList();
